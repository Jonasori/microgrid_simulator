...
def generate_usage_data(node, day_start="20190701", n_days=10, node_type=None, plot=False):
    """
    Units are kW rn
    ...
    if node_type == "OldHome":
        ...
        day_template["demand"] = day_template["Demand (MWh)"]/day_template["Demand (MWh)"].max()
        ...
        node_demand = np.hstack([node_demand_template  + np.random.uniform(-0.03, 0.03, len(day_demand_template)) for _ in range(n_days)])
    ...
    out = pd.DataFrame({"timestamp": ts, "demand": node_demand, "generation": node_generation})
